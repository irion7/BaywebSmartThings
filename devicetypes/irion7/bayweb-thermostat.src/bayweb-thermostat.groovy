 ...
 */
metadata {
	definition (name: "Bayweb Thermostat", namespace: "irion7", author: "Josh Leavitt") {
		capability "Actuator"
	...
	tiles {
		...
		standardTile("switchHold", "device.hold", inactiveLabel: false, decoration: "flat") {
			state "resume", action:"switchHold", nextState: "updating", label:'Hold', icon:"st.samsung.da.oven_ic_send"
		...
		main "temperature"
		details(["temperature", "upButtonControl", "thermostatSetpoint", "currentStatus", "downButtonControl", "mode", "fanMode","humidity", "switchHold", "refresh"])
	}
...
def refresh() {
	...
	poll()
	log.debug "refresh ended data $data device $device"
}
...
def parseEventData(results) {
	log.debug "parseEventData $resluts"
}
...
private getThermostatDescriptionText(name, value, linkText) {
	log.debug "getThermostatDescriptionText $name $value"
	if(name == "temperature") {
...
void switchHold() {
	log.debug "switchHold() is called"
	sendEvent("name":"thermostatStatus", "value":"resuming schedule", "description":statusText, displayed: false)
	def deviceId = device.deviceNetworkId.split("-").last()
	if (parent.releaseHold(deviceId)) {
		sendEvent("name":"thermostatStatus", "value":"setpoint is updating", "description":statusText, displayed: false)
		runIn(5, "poll")
		log.debug "switchHold() is done"
		sendEvent("name":"switchHold", "value":"resume", descriptionText: "switchHold is done", displayed: false, isStateChange: true)
	} else {
		sendEvent("name":"thermostatStatus", "value":"failed resume click refresh", "description":statusText, displayed: false)
		log.error "Error switchHold() check parent.switchHold(deviceId)"
	}
...
/* parent functions
setMode(mode)  off, heat, cool
setFanMode(fanMode)
setHold(holdMode)
setCoolSetpoint(setpoint)
etHeatSetpoint(setpoint)
setActiveActivity(activity)
*/

def cool() {
	log.debug "cool $device"
	def deviceId = device.deviceNetworkId.split("-").last()
